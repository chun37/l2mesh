...
| `l2mesh agent` | 常駐デーモン。直接 peer に gossip して overlay graph を学習、MST を計算、自ノードの BUM FDB を MST の枝に絞る。動作中は BUM FDB を agent が所有 (`tree_neighbor` 無視) |
...
### BUM ツリー (動的 = Phase 2 `l2mesh agent`)

`l2mesh agent` を常駐させると、tinc の switch mode と同じく **動的スパニングツリー** が自動構築される:

- 各 agent が overlay の TCP `4444` で `/info` `/topology` を serve (WG レイヤーが認証境界)
- 5 秒間隔で直接 peer の `/info` を poll、topology を集約
- Kruskal で MST 計算 (重み 1、tiebreak は overlay IP の辞書順なので全ノード同じ MST に収束)
- 自ノードの MST 枝に基づいて vxlan BUM FDB (`00:00:00:00:00:00` エントリ) を書き換え
- WG handshake が 3 分以上途絶えた peer は live 扱いを外して MST から除外
- 90 秒以上 gossip しなかった node は topology から除外
...
[Unit]
Description=L2 Mesh agent (gossip + auto MST)
...
agent と Phase 1 (手動 `tree_neighbor`) は排他: agent 稼働中は agent が BUM FDB を所有し `tree_neighbor` を無視する。auto を使うなら全ノードで agent を有効化、手動運用なら全ノードで agent を止めて `tree_neighbor` を揃える。
