...
> **状態: MVP (alpha)** — 現在は WireGuard レイヤーのピア管理のみ動作する。VXLAN / FRR / EVPN / 昇格降格・死活監視は未実装（[Issues](https://github.com/chun37/l2mesh/issues) 参照）。
...
| `l2mesh root add --name N --pubkey K --endpoint E --ip I` | Root を追加（WireGuard ピア + persistent keepalive 25s）|
| `l2mesh root remove --name N` | Root を削除 |
...
| `l2mesh sync` | state.json から kernel WG にピア反映（boot 時 systemd 用、`ReplacePeers`）|
