...
- **Leaf モードの WG AllowedIPs は未対応**: 現在 `peerConfig` は `/32` 固定なので、Leaf として Primary Root に catch-all (`100.64.0.0/24`) を向けるロジックがない。`l2mesh promote` / `demote` は role flag と FRR/VXLAN の切り替えだけで、Leaf 経由で他 Leaf に到達する経路まで自動構成しない
- フェイルオーバー監視・peer migrate・死活監視ループは未実装
- テスト未整備
